...
import (
	...
	"github.com/kkrishguptaa/whitestone-capital/apps/api/ent/migrate"
...
// QueryUser queries the user edge of a Stake.
func (c *StakeClient) QueryUser(s *Stake) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := s.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(stake.Table, stake.FieldID, id),
...
// QueryStock queries the stock edge of a Stake.
func (c *StakeClient) QueryStock(s *Stake) *StockQuery {
...
// QueryStakes queries the stakes edge of a Stock.
func (c *StockClient) QueryStakes(s *Stock) *StakeQuery {
	query := (&StakeClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := s.ID
		step := sqlgraph.NewStep(
...
// QueryStakes queries the stakes edge of a User.
func (c *UserClient) QueryStakes(u *User) *StakeQuery {
	query := (&StakeClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Stake, Stock, User []ent.Hook
	}
	inters struct {
		Stake, Stock, User []ent.Interceptor
	}
)
